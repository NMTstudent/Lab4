...
## Задание 2
### Построить граффики зависимости количества ошибок от количества эпох обучения. Указать от чего зависит необходимое количество эпох обучения.

Было проведено по 5 эксперементов (одна серия) для каждого количества эпох обучения (от 1 до 5 серий эксперементов для OR, от 1 до 7 для остальных).
...
На основании таблицы создавался соответствующй граффик (по горизонтали - количество эпох обучения, по вертикали - среднее значение итоговой ошибки): 
...
В сопутствующей данной лабораторной работе лекции было сказано, что перцептрон неспособен обучиться быть элементом XOR, решать нелинейные задачи, результаты серий проделанных эксперементов это подтверждают. При этом, из графика видно, что с увеличением количества эпох обучения количество ошибок возрастает и достигает значения 4,  все коофициенты весов перцептрона не подходят для вычислений XOR.

Необходимое количество эпох обучения не зависит от модуля велечин в тренеровочных данных (безразницы, берём ли 10, или 1000), т.к. разница между нужным и реальным выходами учитывается при пересчёте весовых коофициентов. По этой же причине необходимое количество эпох обучения не зависит от случайности.

Предполагаю, что количество необходимых эпох обучения зависит от сложности элемента, до которого перцептрон должен обучиться. 
OR "проще", чем AND или NAND, поэтому для его обучения необходимо меньше эпох обучения. 
OR (Логическое ИЛИ) может быть представлено в виде выражения Х + У, где Х и У имеют значения 0 или 1.
AND и NAND - У * Х и -(У * Х). Для обучения AND и NAND необходимо одиноаковое количество эпох обучения, 
потому что они "родственны" и могут быть выражены через друг друга У * Х = -(-(У * Х)).
